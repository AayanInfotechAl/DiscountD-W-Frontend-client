  ...
  const fetchPrivacyPolicy = async () => {
    ...
    try {
      const response = await axios.get(
        "https://www.discountdoorandwindow.com/api/StaticContent/content/Policy"
      );
      if (response?.data?.status === 200 && response?.data?.success === true) {
        setPrivacyPolicyContent(response?.data?.data);
      }
    } catch (error) {
      console.log(error);
    } finally {
    ...
    <Container className="mt-5 mb-5">
      {loading ? (
        ...
        <div className="row">
          <div className="col">
            <Box mb={3}>
              {privacyPolicyContent && privacyPolicyContent.content ? (
                <>
                  <Box mb={4}>

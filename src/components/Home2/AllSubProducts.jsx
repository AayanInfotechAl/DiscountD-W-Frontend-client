  ...
  const fetchSearchResults = async (query, productId) => {
    ...
    try {
      const response = await axios.get(
        `https://www.discountdoorandwindow.com/api/products/search?name=${query}&category_id=${productId}`
      );
      if (response?.data?.status === 200) {
        setSubsubCategories(response?.data?.data || []);
        setErrorMessage("");
              ...
              <Grid container spacing={2}>
                {subsubCategories?.map((category, index) => (
                  <Grid item xs={12} sm={6} md={4} key={index}>
                    <Box sx={{ borderRadius: "10px", overflow: "hidden", textAlign: "center", position: "relative", backgroundColor: "#f1f1f1", width: "100%", }} className="rounded-3 p-2" onClick={() => handleClick(category)}>
                      ...
                      <p className="fw-bold mb-0">
                        ...
                        <IconButton
                          onClick={(e) => { e.stopPropagation(); handleWishList(category?._id); }}
                        >
                          <FavoriteBorderIcon />
                        </IconButton>
                      </p>
